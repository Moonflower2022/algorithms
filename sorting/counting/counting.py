...
def counting_sorted(arr):
    ...
    types = [type(ele) == int for ele in arr]

    if not all(types):
        raise Exception("wrong type in input array")
    
    # arr only has type int
...
for arr in unsorted:
    print("array: ", arr)
    assert counting_sorted(arr) == sorted(arr), f"oopsie poopie the lists are not the same :( my sort: {counting_sorted(arr)} != python sort: {sorted(arr)}"
